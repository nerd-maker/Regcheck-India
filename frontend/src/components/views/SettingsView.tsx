...
export default function SettingsView({ section }: { section: 'settings' | 'apikeys' }) {
  ...
  const [tab, setTab] = useState<'general' | 'team' | 'security' | 'integrations'>(section === 'apikeys' ? 'integrations' : 'general')

  return (
    <div data-testid={`view-${section}`}>
      <PageHeader
        crumbs={[{ label: 'Workspace', onClick: () => setActiveView('home') }, { label: section === 'apikeys' ? 'API & Vaults' : 'Settings' }]}
      ...
      <div style={{ padding: 24, maxWidth: 720 }}>
        ...
        {tab === 'team' && (
          <div className="rc-card">
            <div className="rc-card-header"><span>Members</span><button className="rc-btn rc-btn-primary rc-btn-sm"><i className="ti ti-user-plus"/> Invite</button></div>
        ...
        {tab === 'integrations' && (
          <div className="rc-card">
            <div className="rc-card-header"><span>Integrations & API keys</span></div>
            <div className="rc-card-body">
              <div style={{ fontSize: 12, color: 'var(--rc-text-muted)', marginBottom: 12 }}>
                Connect AI models, e-Signature providers, and Health Authority gateways.
              </div>
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: '1px solid var(--rc-divider)' }}>
                  ...
                  <button className="rc-btn rc-btn-sm">{it.status === 'Connected' ? 'Manage' : 'Connect'}</button>
                </div>
              ))}
            </div>
          </div>
