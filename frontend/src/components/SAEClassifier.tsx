...
export default function SAEClassifier() {
  ...
  return (
    <div className="space-y-6">
      ...
      {result && (
        <div className="space-y-6">
          {/* DUPLICATE DETECTION — show at top, most urgent */}
          {result.duplicate_detection && (
            <div className={`rounded-2xl border px-6 py-5 mb-4 ${
              result.duplicate_detection.duplicates_found 
                ? 'border-red-500/30 bg-red-500/10' 
                : 'border-green-500/30 bg-green-500/10'
            }`}>
              <div className="flex items-center gap-3 mb-3">
                <div className={`w-3 h-3 rounded-full ${
                  result.duplicate_detection.duplicates_found ? 'bg-red-500' : 'bg-green-500'
                }`}></div>
                <div className="font-semibold text-sm">
                  {result.duplicate_detection.duplicates_found 
                    ? `⚠ ${result.duplicate_detection.match_count} Potential Duplicate(s) Detected`
                    : '✓ No Duplicates Found — Case Appears Unique'
                  }
                </div>
              </div>
              
              <div className="text-sm text-slate-300 mb-3">
                {safeRender(result.duplicate_detection.recommendation)}
              </div>

              {result.duplicate_detection.duplicates_found && 
               Array.isArray(result.duplicate_detection.matches) && (
                <div className="space-y-3 mt-4">
                  <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Matched Cases</div>
                  {result.duplicate_detection.matches.map((match: {
                    case_id: string
                    ...
                    <div key={i} className="bg-white/5 rounded-xl p-4 border border-red-500/20">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-bold text-red-400">{safeRender(match.case_id)}</span>
                        <span className="text-xs font-bold text-red-400 bg-red-500/20 px-2 py-1 rounded-full">
                          {safeRender(match.similarity_score)}% Match
                        </span>
                      </div>
                      <div className="text-xs text-slate-400 mb-2">
                        Category: {safeRender(match.primary_category)} · 
                        Submitted: {safeRender(match.submitted_at)}
                      </div>
                      <div className="text-xs text-slate-500 italic">
                        &quot;{safeRender(match.snippet)}&quot;
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="border-b border-white/10 pb-4 mb-6 mt-4">
              <h2 className="text-xl font-bold uppercase tracking-wider text-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`status-chip text-sm font-medium ${result.requires_expedited_reporting ? 'text-red-400 bg-red-400/10' : 'text-slate-400 bg-white/5'}`} style={{ padding: '4px 12px' }}>
                    {result.requires_expedited_reporting ? 'EXPEDITED REPORTING REQUIRED' : 'ROUTINE REPORTING'}
                  </span>
                </div>
              </h2>
            </div>
              ...
              <div>
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-4">
                  <div>
                    ...
                    <div className="text-2xl font-bold mt-1 text-slate-100">{safeRender(result.primary_category)}</div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <div className="text-xs text-slate-400">Confidence</div>
                      <div className="text-lg font-semibold mt-1">{safeRender(result.confidence)}%</div>
                    </div>
                    <div>
                      <div className="text-xs text-slate-400">Priority Score</div>
                      <div className="text-lg font-semibold mt-1">{safeRender(result.priority_score)} / 10</div>
                    </div>
                  </div>
                  <div>
                    <div className="text-xs text-slate-400">Expedited Reporting</div>
                    <div className="text-lg font-semibold mt-1">{result.requires_expedited_reporting ? 'YES' : 'NO'}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    {Object.entries(result.causality).filter(([k]) => !['assessment','who_umc_category','rationale'].includes(k)).map(([key, value]) => (
              ...
              {result.reporting_timeline && (
                <div>
                  ...
                  <div className="rounded-xl border-l-2 border-indigo-500 bg-white/5 p-4 space-y-3">
                    {Object.entries(result.reporting_timeline).map(([key, value]) => (
                      <div key={key}>
              ...
              <div className="mb-6">
                ...
                <div className="rounded-xl bg-amber-500/10 p-4 border border-amber-500/20">
                  <ol className="list-decimal list-inside space-y-2 text-sm text-amber-200">
                    {result.regulatory_actions_required.map((action: string, i: number) => (
            ...
            {result.audit_log && (
              <div className="mt-4 border-t border-white/10 pt-4">
                <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500">
                  ...
                  <span>•</span>
                  {result.audit_log.study_drug && (
                    <>
                      <span className="font-semibold">STUDY DRUG:</span>
                      <span className="text-teal-400">{safeRender(result.audit_log.study_drug)}</span>
                      <span>•</span>
                    </>
                  )}
