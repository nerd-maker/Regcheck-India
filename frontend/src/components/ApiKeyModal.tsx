    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(6px)' }}>
      ...
      >
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">
                {isChanging ? 'Update API Key' : 'Enter Your Anthropic API Key'}
              </h2>
              <p className="text-xs text-slate-400 mt-0.5">Required to use RegCheck-India agents</p>
            </div>
          </div>
          ...
          {!isChanging && (
            <div className="rounded-xl p-4 text-sm" style={{ background: 'rgba(121,225,214,0.08)', border: '1px solid rgba(121,225,214,0.2)' }}>
              <p className="text-slate-300 leading-relaxed">
                RegCheck-India uses your personal Anthropic API key for all AI agent calls. This means <span className="text-white font-semibold">your credits are used</span>, not ours — giving you unlimited, private access.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="px-8 pb-8 flex items-center gap-3">
          ...
          >
            {isChanging ? 'Update Key' : 'Save & Continue'}
          </button>
          {isChanging && onClose && (
            <button
              onClick={onClose}
